...
//go:build android || ios || ios || ios || ios || ios || ios || ios || ios || ios || ios || ios || ios || ios || ios || ios || ios
...
func (r *Renderer_GLES32) DebugInfo() string {
	return "" // No OOM tracking â€” GLES driver manages memory
}
...
func (r *Renderer_GLES32) Init() {
	...
	if r.enableModel {
		if r.enableShadow {
			...
			// gl.BindFramebuffer(gl.FRAMEBUFFER, r.fbo_shadow)
			// Leave the depth attachment empty here â€” attach per-face with FramebufferTexture2D()
...
func (r *Renderer_GLES32) prepareShadowMapPipeline(bufferIndex uint32) {
	...
	// Removed gl.Enable(gl.TEXTURE_2D) â€” not needed / invalid in GLES3 core
